...
def read_image(filename: str) -> np.ndarray:
  ...
  #extension = os.path.splitext(filename)[1]
    ...
    #image_data = tfio.experimental.image.decode_tiff(image_data, index=0, name=None)
   # image_data = Image.open(filename)
    #image_data = tfio.experimental.image.decode_tiff_info(image_data)
   # image_np = np.array(image_data)
   # image_numpy = tf.convert_to_tensor(image_np, dtype=tf.float32)
  #else:
  image_data = tf.io.read_file(filename)
...
def write_image(filename: str, image: np.ndarray) -> None:
  ...
  #image_in_uint8 = image_in_uint8[:, :, 0]
...
#   image_data = tf.io.read_file(filename)
#   image = tf.io.decode_image(image_data, channels=3)
#   image_numpy = tf.cast(image, dtype=tf.float64).numpy()
#   return image_numpy
#
#
# def write_image(filename: str, image: np.ndarray) -> None:
...
#   #image_in_uint8 = (image_in_uint8_range + 0.5).astype(np.uint8)
#
#   image_in_uint16_range = np.clip(image, 0.0, _UINT8_MAX_F)
#   image_in_uint16 = (image_in_uint16_range + 0.5).astype(np.uint16)
#
#   extension = os.path.splitext(filename)[1]
#
#   if extension == '.jpg':
...
#   else:
#     #image_data = tf.io.encode_png(image_in_uint8)
#     grayscale_image = tf.image.rgb_to_grayscale(image_in_uint16)
#     image_data = tf.io.encode_png(grayscale_image)
#
#   tf.io.write_file(filename, image_data)
