...
def train_loop(
  ...
  if eval_datasets is not None:
    ...
    eval_summary_writer = tf.summary.create_file_writer(eval_folder)
      ...
      logging.info('eval_dataset is None')
  ...
  print("Num GPUs Available: ", len(tf.config.list_physical_devices('GPU'))) #FF
  ...
  while optimizer.iterations.numpy() < num_iterations:
    #logging.info('Shape of stacked images %s', train_set.shape)
